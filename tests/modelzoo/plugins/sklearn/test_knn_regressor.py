...
from molflux.modelzoo.models.sklearn.knn_regressor import KNNRegressor
...
def test_is_mapped_to_correct_class(fixture_model):
    ...
    model = fixture_model
    assert isinstance(model, KNNRegressor)
...
def test_dataset_too_small(fixture_model):
    """Test that a model raises an error if trained on too few data points"""

    model = fixture_model
    small_dataset = train_df[:3]

    with pytest.raises(
        RuntimeError,
        match=r"The training data has 3 points, but the "
        r"n_neighbors parameter of the model is 5*",
    ):
        model.train(train_data=small_dataset)
