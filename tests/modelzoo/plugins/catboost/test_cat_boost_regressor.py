...
from molflux.modelzoo.models.catboost.catboost_regressor import CatBoostRegressor
...
model_name = "cat_boost_regressor"
...
def test_implements_prediction_interval_protocol(fixture_model):
    """That the model implements the PredictionInterval protocol."""
    model = fixture_model
    assert isinstance(model, SupportsPredictionInterval)


def test_implements_standard_deviation_protocol(fixture_model):
    """That the model implements the StandardDeviation protocol."""
    model = fixture_model
...
def test_raises_on_multitask_if_loss_function_is_rmse_with_uncertainty():
    """That the model cannot be initialised with multiple y features if the
    loss function config parameter is set to RMSEWithUncertainty.
    """
    y_features = ["y1", "y2"]
    with pytest.raises(ValueError, match=r".* single task .*"):
        load_model(
            model_name,
            x_features=["x1"],
...
def test_predict_with_prediction_interval(fixture_model, train_data, predict_data):
    """Test that a model can predict with prediction intervals"""
    model = fixture_model
...
def test_predict_with_standard_deviation(fixture_model, train_data, predict_data):
    """Test that a model can predict with standard deviatian"""
    model = fixture_model

    model.train(train_data=train_data)
    assert supports_std(model)
    assert model.predict_with_std(data=predict_data)
