...
model_name = "average_features_regressor"
...
def test_is_mapped_to_correct_class(fixture_model):
    ...
    model = fixture_model
    assert isinstance(model, AverageFeaturesRegressor)
...
@pytest.mark.parametrize(
    "train_data, predict_data, expected",
    [
        (train_df, predict_df, [3 / 2, 2, 3]),
        (
...
def test_train_predict_model(fixture_model, train_data, predict_data, expected):
    ...
    for task_predictions in predictions.values():
        ...
        assert task_predictions == pytest.approx(expected=expected)
...
@pytest.mark.parametrize(
    "train_data, predict_data, expected_std",
    [
        (train_df, predict_df, [0.5, 1.0, 1.0]),
        (
            datasets.Dataset.from_pandas(train_df),
            datasets.Dataset.from_pandas(predict_df),
            [0.5, 1.0, 1.0],
...
def test_train_predict_with_standard_deviation(
    fixture_model,
    train_data,
    predict_data,
    expected_std,
    ...
    for prediction_std in stds.values():
        ...
        assert prediction_std == pytest.approx(expected=expected_std, abs=0.001)
...
def test_train_predict_with_standard_deviation_gaussian_mixture_approximation(
    fixture_model,
    train_data,
    predict_data,
    expected_std,
