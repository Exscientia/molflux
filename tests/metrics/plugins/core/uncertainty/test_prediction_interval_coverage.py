...
from molflux.metrics.uncertainty.prediction_interval_coverage import (
...
@pytest.fixture()
def fixture_metric():
    return load_metric("prediction_interval_coverage")
...
def test_metric_in_catalogue():
    ...
    assert "prediction_interval_coverage" in all_metric_names
...
def test_metric_is_mapped_to_correct_class(fixture_metric):
    ...
    assert isinstance(metric, PredictionIntervalCoverage)
...
def test_default_compute(fixture_metric):
    ...
    assert result["prediction_interval_coverage"] >= 0
...
def test_standard_deviations(fixture_metric):
    ...
    expected = 3 / 4
    assert np.isclose(result["prediction_interval_coverage"], expected, atol=0.01)
