...
from molflux.metrics.classification.roc_auc import RocAuc
...
def test_metric_is_mapped_to_correct_class(fixture_metric):
    ...
    assert isinstance(metric, RocAuc)
...
def test_binary_classification(fixture_metric):
    ...
    assert pytest.approx(result[metric_name], 0.01) == 0.78


def test_multiclass_classification():
    """That can score multiclass classification."""
    metric = load_metric(metric_name, config_name="multiclass")
    ...
    assert metric_name in result
    assert pytest.approx(result[metric_name], 0.01) == 0.85
...
def test_multilabel_classification():
    ...
    assert metric_name in result
    assert np.allclose(result[metric_name], [0.83, 0.38, 0.94], rtol=0, atol=1e-2)
