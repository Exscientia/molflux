...
from molflux.features.representations.openeye.descriptors.rotatable_bonds import (
    RotatableBonds,
)

representation_name = "rotatable_bonds"
...
def test_representation_is_mapped_to_correct_class(fixture_representation):
    ...
    assert isinstance(representation, RotatableBonds)
...
def test_default_digitise(fixture_representation):
    ...
    expected_result = [[0, 0, 1, 0, 0, 0, 0, 0, 0]]
    assert representation_name in result
    assert result[representation_name] == expected_result
