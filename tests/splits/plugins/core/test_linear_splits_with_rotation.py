...
strategy_name = "linear_split_with_rotation"
...
def test_indices_rotation(fixture_sample_dataset, fixture_test_strategy):
    ...
    unrot_indices = strategy.split(dataset=dataset, n_splits=1)
    train_indices_unrot, validation_indices_unrot, test_indices_unrot = next(
        unrot_indices,
    )
    train_indices_rot = (train_indices_unrot + ceil(len(dataset) / 2)) % len(dataset)
    validation_indices_rot = (validation_indices_unrot + ceil(len(dataset) / 2)) % len(
        dataset,
    )
