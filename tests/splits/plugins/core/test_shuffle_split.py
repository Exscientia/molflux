...
def test_default_split_fractions(fixture_sample_dataset, fixture_test_strategy):
    ...
    assert len(list(train_indices)) == int(0.8 * n_samples)
    assert len(list(validation_indices)) == int(0.1 * n_samples)
    assert len(list(test_indices)) == math.ceil(0.1 * n_samples)
...
def test_custom_split_fractions(fixture_sample_dataset, fixture_test_strategy):
    ...
    train_indices, validation_indices, test_indices = next(indices)
    assert len(list(train_indices)) == int(0.6 * n_samples)
    assert len(list(validation_indices)) == int(0.3 * n_samples)
    assert len(list(test_indices)) == math.ceil(0.1 * n_samples)


def test_non_deterministic_split(fixture_sample_dataset, fixture_test_strategy):
...
def test_deterministic_split_if_seed_set(fixture_sample_dataset, fixture_test_strategy):
    ...
    indices1 = strategy.split(dataset=dataset, seed=123)
...
def test_splits_are_shuffled_across_folds(
    ...
    indices1 = strategy.split(dataset=dataset, n_splits=2, seed=123)
    train_indices1, validation_indices1, test_indices1 = next(indices1)
...
def test_no_sample_in_validation(fixture_test_strategy):
    """
    A default train test split of a dataset with 1126 molfluxs was allocating
    one sample into the validation set, which should instead be empty.
    """
    dataset = range(1126)
    strategy = fixture_test_strategy
    indices = strategy.split(
    ...
    train_indices, validation_indices, test_indices = next(indices)
    assert len(list(train_indices)) == 900
    assert len(list(validation_indices)) == 0
    assert len(list(test_indices)) == 226
